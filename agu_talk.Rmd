...
BASE_DIR <- "/Users/dorh012/Documents/2021/FoRTE/2021_FoRTE_AGU"
...
monthly_met %>% 
  ...
  mutate(description = paste0(str_to_title(description), '\n(', units, ')')) %>% 
  mutate(met = as.factor(met)) -> 
  data 

data %>% 
  filter(variable %in% c("tmp", "prate")) -> 
  data

ggplot() + 
  geom_line(data = data, aes(mon_time, value, group = met, line = met, color = met), 
            alpha = 0.8) +
   facet_wrap("description", scales = 'free', strip.position = "top",
             labeller = label_wrap_gen(width = 20), ncol = 1) +
  labs(y = NULL, x  = NULL) +
  THEME +
...
ggsave(plot, filename = "figures/monthly_met.png", width = 3, height = 6)
```

```{r}
data %>%
  group_by(met, variable, description) %>% 
  summarise(value = mean(value)) %>%  
  mutate(join = 1) -> 
  average_values

data.frame(year = 1900:2050, 
           join = 1) %>% 
  inner_join(average_values) %>% 
  select(-join) %>% 
  ggplot() + 
  geom_line( aes(year, value, group = met, line = met, color = met), 
            alpha = 0.8) +
    facet_wrap("description", scales = 'free', strip.position = "top",
             labeller = label_wrap_gen(width = 20), ncol = 1) +
  labs(y = NULL, x  = NULL) +
  THEME +
  scale_x_continuous(labels = c("1900", "1950", "2000", "2050"), breaks = c(1900, 1950, 2000, 2050)) +
  theme(legend.position = "none") -> 
  plot

ggsave(plot, filename = "figures/year.png", width = 3, height = 6)
...
ggsave(plot, filename = "figures/resistance.png", width = 8, height = 2)
```
...
ggplot() + 
  ...
  ggsave(plot, filename = "figures/resilience.png", width = 8, height = 2)
```